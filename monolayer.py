...
class monolayer(Component):
    ...
    def __init__(self, apm, b_heads, vm_heads, thickness_heads,
                 b_tails, vm_tails, thickness_tails, roughness, solv_rough, nWater = 0,
        ...
        self.solv_rough = possibly_create_parameter(
            solv_rough,
            name='solvent roughness')
        ...
        self.nWater = possibly_create_parameter(nWater, 'nb waters')
    ...
    def __repr__(self):
        ...
        s = (" Monolayer({apm!r}, {bh!r}, {vm_heads!r}, {thickness_heads!r},"
             " {bt!r}, {vm_tails!r}, {thickness_tails!r}, {roughness!r}, {solv_rough!r},"
             " head_solvent={head_solvent!r}, nWater={nWater!r}"
    ...
    def slabs(self, structure=None):
        ...
        self.thickness_heads.value = (self.vm_heads.value + self.nWater.value*29.9)/self.apm.value
        ...
        #Volume fraction
        #volfrac = 1 - ((self.thickness_tails.value*self.vm_heads.value) / (self.vm_tails.value*self.thickness_heads.value))

        volfrac = self.vm_heads.value/(self.thickness_heads.value * self.apm.value)

        # SLD's
        sld_heads = volfrac*(float(self.b_heads_real.value) / float(self.vm_heads.value) * 1.e6) + (1 - volfrac)*self.head_solvent
        ...
        # roughnesses
        layers[0, 3] = float(self.solv_rough)
